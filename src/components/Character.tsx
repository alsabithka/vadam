...
export function useCharacterAnim({
  ...
  const windup = useMotionValue(0)
  const breath = useMotionValue(0)
  const torsoRot = useTransform(
    () => bodyLean.get() + windup.get() + breath.get(),
  )
  const clothRot = useSpring(torsoRot, {
    stiffness: 55,
    damping: 11,
    mass: 1.2,
  })
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const speed =
        (straining ? 1.5 : 0.7) + i * 3.4 + Math.min(2.4, dragged * 5)
      ...
      breath.set(
        Math.sin((now / 1000) * (1.05 + phaseOffset * 0.1)) * (1.1 + i * 0.7),
      )
  ...
  const localHandX = useTransform(torsoRot, (deg) => {
    const rad = deg * (Math.PI / 180)
    const dx = 192 - 112
    const dy = 168 - 200
    return 112 + dx * Math.cos(rad) - dy * Math.sin(rad)
  })

  const localHandY = useTransform(torsoRot, (deg) => {
    const rad = deg * (Math.PI / 180)
    const dx = 192 - 112
    const dy = 168 - 200
    return 200 + dx * Math.sin(rad) + dy * Math.cos(rad)
  })
...
export interface CharacterLayerProps {
  team: Team
  side: "left" | "right"
  anim: ReturnType<typeof useCharacterAnim>
  x: number
  y: number
}
...
function LayerWrapper({
  ...
  return (
    <g
      transform={`translate(${x}, ${y}) scale(${side === "right" ? -1 : 1}, 1)`}
    >
      {children}
    </g>
  )
...
export function CharacterBackLeg({
  team,
  side,
  anim,
  x,
  ...
  return (
    <LayerWrapper x={x} y={y} side={side}>
      ...
      <motion.g style={backLegStyle}>
        ...
        />
        <path
          d="M96 196 q-16 20 -22 40 l24 6 q6 -24 20 -40 z"
          fill={MUNDU_SHADE}
        />
        <path
          d="M70 292 q-14 4 -18 12 q-2 8 10 10 l30 0 q8 -1 6 -10 l-6 -14 z"
          fill={SKIN}
        />
      </motion.g>
    </LayerWrapper>
...
export function CharacterTorso({
  team,
  side,
  anim,
  x,
    ...
    <LayerWrapper x={x} y={y} side={side}>
      ...
      <motion.g style={{ ...originTorso, rotate: anim.torsoRot }}>
        ...
        <g transform="rotate(-6 150 70)">
          ...
          <path
            d="M133 66 q0 -22 20 -24 q22 -2 20 18 q-10 -8 -22 -6 q-14 2 -18 12 z"
            fill="#241209"
          />
        ...
        <g transform="translate(192 168)">
          ...
          <path
            d="M-8 -6 q10 -6 18 2 M-9 0 q11 -4 19 3 M-8 6 q10 -3 18 3"
            stroke={SKIN_SHADE}
            strokeWidth="2.4"
            fill="none"
            strokeLinecap="round"
        ...
        <g transform="translate(190 180)">
          ...
          <path
            d="M-8 -4 q10 -5 17 2 M-8 3 q10 -3 17 3"
            stroke="#6f4520"
            strokeWidth="2.2"
            fill="none"
            strokeLinecap="round"
...
export function CharacterFrontLeg({
  team,
  side,
  anim,
  x,
  ...
  return (
    <LayerWrapper x={x} y={y} side={side}>
      ...
      <motion.g style={frontLegStyle}>
        ...
        <path
          d="M132 296 q22 2 30 12 q4 9 -8 11 l-34 0 q-8 -1 -6 -12 l4 -12 z"
          fill={SKIN}
        />
      </motion.g>
    </LayerWrapper>
...
// Needed because CharacterBackLeg uses it
...
export default function Character() {
  // kept for backward compatibility if needed, but not used in new design
  return null
}
