...
export default function Win({ winner, role, playerName, opponentName, onPlayAgain, onNewRoom }: WinProps) {
  const iWon = (winner === 'host' && role === 'host') || (winner === 'guest' && role === 'guest')
  const winnerName = winner === 'host' ? (role === 'host' ? playerName : opponentName) : role === 'guest' ? playerName : opponentName
    ...
    <div className="relative h-full w-full overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-black/45 px-6">
        <div className="animate-rise flex flex-col items-center gap-6 text-center">
          ...
          <h1 className="font-display text-5xl font-extrabold text-cream" style={{ textShadow: '0 3px 0 #6b3f22' }}>
            {winnerName} wins!
          </h1>
          <p className="font-body text-base text-cream/80">
            {iWon ? 'You out-shouted the field. Onam glory!' : 'So close — shout louder next round.'}
          </p>
