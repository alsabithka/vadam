...
export default function App() {
  ...
  const beginOnline = async (role: 'host' | 'guest', name: string, code: string) => {
    setError(null)
    if (!(await isSupabaseConfigured())) {
      setError('Online play needs Supabase Realtime. Connect it, or play Practice for now.')
      return
    }
    setSession({
      role,
  ...
  const handleJoin = (name: string, code: string) => beginOnline('guest', name, code)
  ...
  return (
    <div className="h-full w-full">
      {scene === 'landing' && (
        <Landing onHost={handleHost} onJoin={handleJoin} onPractice={handlePractice} error={error} />
      )}
      ...
      <div className="landscape-hint fixed inset-0 z-50 flex-col items-center justify-center gap-3 bg-mud-deep px-8 text-center">
        ...
        <p className="font-display text-2xl font-bold text-cream">Rotate your device</p>
        <p className="font-body text-sm text-cream/70">Vadam Vali is best played in landscape.</p>
      </div>
    </div>
  )
}
